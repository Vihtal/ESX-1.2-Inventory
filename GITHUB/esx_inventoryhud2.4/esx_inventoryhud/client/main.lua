...
function closeInventory()
    ...
    if targetPlayer ~= nil then
        TriggerServerEvent('seconds-inventoryhud:StopSearchingPlayer', targetPlayer)
        targetPlayer = nil
...
RegisterCommand('searchplayer', function(source, args)
    ...
    if closestPlayer ~= -1 and dist <= 3.0 then
        ...
        if IsEntityPlayingAnim(playerPed, "missminuteman_1ig_2", "handsup_enter", 3) then
            ...
            TriggerServerEvent('seconds-inventoryhud:openInventory', serverId)
        else
            exports.pNotify:SendNotification(
            {
                text = "The player is not surrendering",
                type = "error",
                timeout = 3000,
                layout = "bottomCenter",
                queue = "inventoryhud"
        ...
        exports.pNotify:SendNotification(
            {
                text = "There's no players nearby",
                type = "error",
                timeout = 3000,
                layout = "bottomCenter",
                queue = "inventoryhud"
...
RegisterCommand('lootplayer', function(source, args)
    local closestPlayer, dist = ESX.Game.GetClosestPlayer()
    if closestPlayer ~= -1 and dist <= 3.0 then
        local serverId = GetPlayerServerId(closestPlayer)
        local player = GetPlayerFromServerId(serverId)
        ...
        if IsPedDeadOrDying(playerPed, 1) then
            TaskStartScenarioInPlace(PlayerPedId(), "PROP_HUMAN_BUM_BIN", 0, true)
					exports['progressBars']:startUI(5000, "Looting")
					Citizen.Wait(5000)
                    ClearPedTasksImmediately(PlayerPedId())
            TriggerServerEvent('seconds-inventoryhud:openInventory', serverId)
        else
